...
<style>
    ...
    /* 強制 DataEditor 框線顯示 */
    [data-testid="stDataEditor"] div[data-testid="stTable"] { border: 1px solid #475569 !important; }
    [data-testid="stDataEditor"] canvas { border: 1px solid #475569 !important; }
    .stDataFrame { border: 1px solid #475569 !important; }
    
    footer { visibility: hidden; }
</style>
""", unsafe_allow_html=True)

# 注入 JS 強制畫出 Canvas 框線
components.html("""
<script>
    const canvas = window.parent.document.querySelectorAll('canvas');
    canvas.forEach(c => {
        c.style.border = "1px solid #475569";
    });
</script>
""", height=0)
...
with tab1:
    ...
    f_note = st.text_area("內容錄入", key=f"n_{rk}", label_visibility="collapsed", height=100)
