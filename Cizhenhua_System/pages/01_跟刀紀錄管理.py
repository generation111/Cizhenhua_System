...
with tab1:
    if "rk_v21" not in st.session_state: st.session_state.rk_v21 = 0
    rk = st.session_state.rk_v21
    ...
    if d_price == "單次批價使用":
        ...
        d_pid_now = st.session_state.get(f"pi_{rk}", "").strip()
        d_prod_now = st.session_state.get(f"pd_{rk}", "")
        cur_bal = get_current_balance(db_df, d_pid_now, d_prod_now)
        if cur_bal > 0:
            c6.success(f"✅ 累計餘量：{cur_bal}")
            d_pre_today = c5.number_input("扣除量", min_value=1, max_value=cur_bal, value=1, key=f"py_{rk}")
            d_qty = d_pre_today
        else:
            c5.warning("請輸入 ID 或餘額為 0")
            can_submit = False
    elif d_price == "純預購寄庫":
        d_pre_total = c5.number_input("預購總量", min_value=1, value=5, key=f"pt_{rk}")
        d_qty = 0
        ...
        if st.button("🚀 提交數據", key="sub_btn", disabled=not can_submit):
            with st.spinner("系統存檔中..."):
                try:
                    ...
                    # 顯示成功並立即刷新
                    status_msg.success("✅ 資料已成功存入雲端試算表！")
                    time.sleep(1)
                    st.cache_data.clear()
                    st.session_state.rk_v21 += 1
                    st.rerun()
                except Exception as e:
                    # 只有真正噴錯才顯示失敗
                    status_msg.error(f"⚠️ 提交不穩定，請檢查歷史紀錄是否有資料。")
