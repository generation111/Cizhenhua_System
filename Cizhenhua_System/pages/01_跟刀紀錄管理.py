...
with tab1:
    if "rk_v13" not in st.session_state: st.session_state.rk_v13 = 0
    rk = st.session_state.rk_v13
        ...
        if not d_pid:
            ...
            if not df_history.empty:
                ...
                if current_balance > 0:
                    ...
                    d_pre_today = c5.number_input(f"扣除量 (Max:{int(current_balance)})", min_value=1, max_value=int(current_balance), value=1, key=f"py_{rk}")
                    d_qty = d_pre_today
                    d_pre_total = 0 # 本次不批價，預購總量設為 0，避免公式累加錯誤
                    c6.info(f"僅扣除，不新增預購")
                else:
                    st.error(f"❌ {d_prod} 餘額為 0，不可選此項")
                    can_submit = False
            else:
                st.info("查無此 ID 資料")
                can_submit = False
