...
<style>
    ...
    .stTextInput input, .stDateInput input, .stNumberInput input {{
        height: 40px !important; font-size: 1.2rem !important; font-weight: 500 !important; color: #1e293b !important;
    }}
    .stTabs [data-baseweb="tab"] {{
        height: 52px !important; background-color: white; font-weight: 800 !important; font-size: 1.25rem !important;
    }}
...
with tab1:
    ...
    if d_price == "使用前次預購":
        ...
        user_df = db_df[(db_df['病例號/ID'].astype(str).str.strip() == p_now) & (db_df['產品項目'] == pr_now)]
        cur_bal = int(user_df.iloc[-1]['預購餘量']) if not user_df.empty else 0
        if cur_bal > 0:
            ...
            c5.warning("餘額不足"); can_submit = False
    elif d_price == "批價 + 預購":
    ...
    if c18.button("🚀 提交數據", key="sub_btn", disabled=not (can_submit and d_pid)):
        # 計算餘額
        temp_df = fetch_all_data()
        prev_df = temp_df[(temp_df['病例號/ID'].astype(str).str.strip() == d_pid.strip()) & (temp_df['產品項目'] == d_prod)]
        bal = int(prev_df.iloc[-1]['預購餘量']) if not prev_df.empty else 0
        
        if d_price == "使用前次預購": final_bal = bal - d_pre_today
        elif d_price in ["批價 + 預購", "純預購寄庫"]: final_bal = bal + (d_pre_total - d_pre_today)
        else: final_bal = 0
